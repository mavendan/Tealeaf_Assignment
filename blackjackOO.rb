...
class CardsGame
    ...
    def shuffle
      cards_deck= self.cards_deck.shuffle
    end   
...
class BlackJack < CardsGame
    ...
    #Let know that you are going to stay with current cards
    def stand
    end
...
class Player
    ...
    attr_accessor :name, :balance, :current_beat
      
    def initialize
      @cards = Array.new(52)
    end
    
    def set_beat_amount beat_amount
    ...
    def get_current_cards_value
      return @cards.inject{ |sum, n| sum + n.to_i  }
    end
...
class Card
    ...
    def to_s
      return "#{self.suite} #{self.number} value:#{self.value} second value: #{self.second_value}"
    end
    
    def to_i
      return self.value.to_is
    end
